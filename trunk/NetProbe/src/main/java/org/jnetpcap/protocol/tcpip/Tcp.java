 ...
 * that it is also the offset from the capture of the TCP segment to the actual
 * data.
...
public class Tcp extends JHeaderMap<Tcp> implements JHeaderChecksum {
	...
	public static class SACK extends TcpOption {
		 ...
		 * 2 element block. The even elements starting at index 0 are capture
	...
	public static abstract class TcpOption extends JSubHeader<Tcp> {
		 ...
		 * @param offset
		 *            offset into the buffer where tcp option header capture (in
		 *            bytes)
	 ...
	 * @param offset
	 *            offset into the buffer where tcp header capture (in bytes)
	 * @return number of bytes occupied by the tcp header, including any tcp
	 *         options
	 */
	@HeaderLength
	 ...
	 * name from the fact that it is also the offset from the capture of the TCP
